...
enum Tool { pen, eraser, color }

class WhiteboardViewModel extends ChangeNotifier {
  WhiteboardViewModel(this._firestore, this._id, this._firestoreDatabasecollectionId) {
    _whiteboardRef = _firestore.collection(_firestoreDatabasecollectionId).document(_id);
    _subscription = _whiteboardRef.snapshots().listen(_onContentSnapshot);
  }

  final Firestore _firestore;
  final String _id;
  final String _firestoreDatabasecollectionId;
  ...
  void onGestureStart(Point point) {
    if (_tool == Tool.pen) {
      _currentLine = Line([point], color.value, type);
      notifyListeners();
    } else {
      _remove(point);
    }
  ...
  void selectTool(Tool tool, context) {
    _tool = tool;
    print(color);
    notifyListeners();
  }
